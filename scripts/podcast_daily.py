...
def check_updates(podcasts, tracker):
    ...
    for podcast in podcasts:
        ...
        if new_episodes:
            ...
            # 保存截止点 = 这次抓到的 new_episodes 中最旧的 episode
            oldest_ep = new_episodes[-1]
        else:
            print(f"    - 无新 episode")
            # 即使没有新episode，也保存当前最新的作为截止点
            # 防止重复获取
            oldest_ep = episodes[0] if episodes else None
        
        # 【关键】无论有没有新episode，都要更新tracker作为边界
        if oldest_ep:
...
def generate_summaries(updates):
    ...
    MAX_EPISODES_PER_PODCAST = 1  # 每个播客源只取最新1条
...
def main():
    ...
    if fetch_only:
        ...
        for update in updates:
            # 每个播客只取最新1条
            for ep in update.get("episodes", [])[:1]:
                pending.append({
                    "id": str(uuid.uuid4()),
                    "title": ep.get("title", ""),
    ...
    if root_path.exists():
        with open(root_path, "r", encoding="utf-8") as f:
            ...
            existing_data = [p for p in existing_raw if p.get('fetched_at','')[:10] >= cutoff]
    
    # 按 source+title 去重，保留最新（fetched_at最新）
    key_map = {}
    for p in summaries:
    ...
    for p in existing_data:
        key = (p.get('source',''), p.get('title',''))
        if key not in key_map or p.get('fetched_at','') > key_map[key].get('fetched_at',''):
            key_map[key] = p
    all_podcasts = list(key_map.values())
    # 按 fetched_at 倒序排列，取最新50条
    all_podcasts.sort(key=lambda x: x.get('fetched_at',''), reverse=True)
    all_podcasts = all_podcasts[:50]
    print(f"📊 合并后播客数量: {len(all_podcasts)}")
