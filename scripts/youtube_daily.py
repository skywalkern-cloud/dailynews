    ...
    for update in updates:
        ...
        for video in videos:
            ...
            results.append({
                ...
                "source": channel_name,
                "channel": channel_name,  # 显式加channel字段
                "url": url,
...
def main():
    ...
    pending_file = MEMORY_DIR / "youtube-pending.json"
    if fetch_only:
        pending = []
        for update in updates:
            # 每个频道只取最新1条
            for video in update.get('videos', [])[:1]:
                pending.append({
                    "id": str(uuid.uuid4()),
                    "title": video.get('title', ''),
                    "source": update.get('channel_name'),
                    "channel": update.get('channel_name'),
                    "url": video.get('url', ''),
                    "upload_date": video.get('upload_date', ''),
        ...
        if pending:
            ...
            print(f"\n⚠️ 无新视频，pending已清空")
        print("下一步运行: python3 scripts/youtube_download.py")
        return
    ...
    # 【FIX】合并数据（URL去重，保留最新摘要），只保留7天内数据
    from datetime import datetime
    def is_recent(item):
        pub = item.get('upload_date') or item.get('pubDate', '') or ''
        if not pub or len(pub) < 8:
            return True  # 无日期默认保留
        try:
            dt = datetime(int(pub[:4]), int(pub[4:6]), int(pub[6:8]))
            return (datetime.now() - dt).days <= 7
        except:
            return True  # 解析失败默认保留
    
    url_map = {}
    for v in summaries + existing_data[:30]:
        if not is_recent(v):
            continue  # 跳过7天前的旧数据
        url = v.get('url', '')
        if url:
            if url not in url_map or v.get('summary'):
                url_map[url] = v
    all_videos = list(url_map.values())
