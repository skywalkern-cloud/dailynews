...
def deploy_to_github():
    ...
    try:
        # 0. 检查是否有未提交的本地变更， stash 备份
        result = subprocess.run(["git", "status", "--porcelain"], cwd=str(WORK_DIR), capture_output=True, text=True)
        had_stash = False
        if result.stdout.strip():
            print("  发现本地变更，执行 stash 备份...")
            stash_result = subprocess.run(
                ["git", "stash", "push", "-m", "backup before deploy"],
                cwd=str(WORK_DIR), capture_output=True, text=True
            )
            if stash_result.returncode != 0:
                print(f"✗ stash 失败: {stash_result.stderr}")
                return False
            had_stash = True
            print("  ✓ 已 stash 本地变更")
        else:
            print("  ✓ 无本地变更，无需 stash")
        ...
        if had_stash:
            ...
            if restore_result.returncode == 0:
                ...
                print(f"  ⚠ 恢复 stash 失败（可手动 git stash pop）: {restore_result.stderr}")
